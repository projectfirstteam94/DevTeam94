<?php
require_once $_SERVER["DOCUMENT_ROOT"] . "/lib/contact/PrivatejetContactMail.php";
use hawaii\contact\PrivatejetContactMail;
?>
...
$to = "test-privatejet@kslink.work";
...
$BccMail = "test-privatejet@mail.kslink.work";

// 管理者宛に送信されるメールのタイトル（件名）
$subject = "プライベートジェットお問い合わせ";
...
$require = array('お名前','フリガナ','メールアドレス','電話番号');
...
if(($confirmDsp == 0 || $sendmail == 1) && $empty_flag != 1){
	...
	//メール送信処理前に内容を保存
	//---------------------------------
	$contactMailObj = new PrivatejetContactMail(
		postToSaveMail($_POST),
		date("Y-m-d H:i:s", time()),
		@$_SERVER["REMOTE_ADDR"],
		getHostByAddr(getenv('REMOTE_ADDR')),
		$confirmDsp != 1 ? @$_SERVER['HTTP_REFERER'] : @$_POST['httpReferer']
	);
	$mailSavedFlg = $contactMailObj->save();
	// 保存に成功したときのみメールを送信する
	if ($mailSavedFlg) {
...
<?php }else if (!$mailSavedFlg) { ?>
<h4>メールの送信処理に失敗しました。「戻る」ボタンにて再送信をお願い致します。</h4>
<br /><br /><input type="button" value=" 前画面に戻る " onClick="history.go(-2)">
</div>
</body>
</html>
<?php }else{ ?>
...
送信は正常に完了しました。<br /><br />
<br />
<strong>こちらより送らせていただくメールを受けられるように、<br />
@hawaiiansky.jp からの受信を可能に設定しておいて下さい。</strong>
<br />
...
//メール内容保存用にPOSTデータをセットする関数
function postToSaveMail($arr){
	global $hankaku,$hankaku_array;
	$resArray = array();
	foreach($arr as $key => $val) {
		$out = '';
		...
		if($out != "confirm_submit" && $key != "httpReferer") {
			$resArray[] = array("name"=>$key, "value"=>$out);
		}
	}
	return $resArray;
}
